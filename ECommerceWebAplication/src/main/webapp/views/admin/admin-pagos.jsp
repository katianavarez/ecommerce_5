...
<html lang="es">
    <head>
        ...
        <title>Pagos — Velour Admin</title>
        ...
        <div class="admin-layout">
            <aside class="admin-sidebar">
                ...
                <nav class="admin-nav">
                    ...
                    <a href="${pageContext.request.contextPath}/admin/pagos" class="admin-nav__link admin-nav__link--active">
            ...
            <div class="admin-content">
                <div class="admin-topbar">
                    <div>
                        <h1 class="admin-topbar__title">Historial de Pagos</h1>
                        <p class="admin-topbar__subtitle"><c:out value="${totalRegistros}"/> pagos en total</p>
                    </div>
                </div>
                ...
                <div class="admin-body">
                    ...
                    <form id="formFiltrosPagos" method="GET" action="${pageContext.request.contextPath}/admin/pagos">
                        <%-- Tabs por estado de pago --%>
                        <div class="admin-tabs" style="margin-bottom:var(--sp-4);">
                            <button type="submit" name="estadoPago" value="TODOS"
                                    class="admin-tab ${estadoSeleccionado == 'TODOS' ? 'admin-tab--active' : ''}">
                                Todos (<c:out value="${conteosPorEstado['TODOS']}"/>)
                            </button>
                            <c:forEach var="est" items="${estadosPago}">
                                <button type="submit" name="estadoPago" value="${est.name()}"
                                        class="admin-tab ${estadoSeleccionado == est.name() ? 'admin-tab--active' : ''}">
                                    <c:choose>
                                        <c:when test="${est.name() == 'PENDIENTE'}">Pendientes</c:when>
                                        <c:when test="${est.name() == 'APROBADO'}">Aprobados</c:when>
                                        <c:when test="${est.name() == 'RECHAZADO'}">Rechazados</c:when>
                        ...
                        <div class="admin-table-head">
                            <h2 class="admin-table-head__title">Pagos</h2>
                            <div class="admin-table-head__controls">
                                ...
                                <select class="admin-select" name="metodoPago" onchange="this.form.submit()">
                                    <option value="TODOS" ${metodoSeleccionado == 'TODOS' ? 'selected' : ''}>Todos los métodos</option>
                                    <c:forEach var="met" items="${metodosPago}">
                                        <option value="${met.name()}" ${metodoSeleccionado == met.name() ? 'selected' : ''}>
                                            <c:choose>
                                                <c:when test="${met.name() == 'TARJETA'}">Tarjeta</c:when>
                                                <c:when test="${met.name() == 'TRANSFERENCIA'}">Transferencia</c:when>
                                                <c:when test="${met.name() == 'CONTRA_ENTREGA'}">Contra entrega</c:when>
                                                <c:otherwise><c:out value="${met}"/></c:otherwise>
                                            </c:choose>
                                        </option>
                                    </c:forEach>
                                </select>
                                <input type="hidden" name="estadoPago" value="${estadoSeleccionado}">
                            </div>
                        </div>
                    </form>
                    ...
                    <div class="admin-table-wrap">
                        <table class="admin-table">
                            <thead>
                                <tr><th># Pago</th><th># Pedido</th><th>Cliente</th><th>Fecha</th><th>Método</th><th>Monto</th><th>Estado</th><th>Acciones</th></tr>
                            </thead>
                            <tbody>
                                <c:choose>
                                    <c:when test="${empty pedidosPagos}">
                                        <tr><td colspan="8" style="text-align:center;color:var(--text-muted);padding:var(--sp-8);">No hay pagos que coincidan con los filtros.</td></tr>
                                    </c:when>
                                    <c:otherwise>
                                        <c:forEach var="ped" items="${pedidosPagos}">
                                            <tr>
                                                <td><strong style="color:var(--text-heading);">#<c:out value="${ped.pago.id}"/></strong></td>
                                                <td>
                                                    <a href="${pageContext.request.contextPath}/admin/pedidos?accion=detalle&id=${ped.id}" style="color:var(--color-secondary);">
                                                        #<c:out value="${ped.numPedido}"/>
                                                    </a>
                                                </td>
                                                <td>
                                                ...
                                                <td style="color:var(--text-muted);"><c:out value="${ped.pago.fecha}"/></td>
                                                <td>
                                                    <c:choose>
                                                        <c:when test="${ped.pago.metodo.name() == 'TARJETA'}">Tarjeta</c:when>
                                                        <c:when test="${ped.pago.metodo.name() == 'TRANSFERENCIA'}">Transferencia</c:when>
                                                        <c:when test="${ped.pago.metodo.name() == 'CONTRA_ENTREGA'}">Contra entrega</c:when>
                                                        <c:otherwise><c:out value="${ped.pago.metodo}"/></c:otherwise>
                                                    </c:choose>
                                                </td>
                                                <td><strong>$<fmt:formatNumber value="${ped.pago.monto}" maxFractionDigits="0"/></strong></td>
                                                <td><span class="badge badge--${ped.pago.estado.name().toLowerCase()}"><c:out value="${ped.pago.estado}"/></span></td>
                                                <td>
                                                    <div class="admin-table__actions">
                                                        <a href="${pageContext.request.contextPath}/admin/pedidos?accion=detalle&id=${ped.id}" class="admin-action-btn" title="Ver pedido">
                                ...
                                <c:if test="${totalPaginas > 1}">
                                    <nav class="pagination" style="margin-top:0;flex-shrink:0;">
                                        <c:if test="${paginaActual > 1}">
                                            <a class="pagination__btn" href="${pageContext.request.contextPath}/admin/pagos?estadoPago=${estadoSeleccionado}&metodoPago=${metodoSeleccionado}&busqueda=${busquedaActual}&pagina=${paginaActual - 1}">&#8249;</a>
                                        </c:if>
                                        <c:forEach var="i" begin="1" end="${totalPaginas}">
                                            <c:choose>
                                                ...
                                                <c:when test="${i == 1 || i == totalPaginas || (i >= paginaActual - 1 && i <= paginaActual + 1)}">
                                                    <a class="pagination__btn" href="${pageContext.request.contextPath}/admin/pagos?estadoPago=${estadoSeleccionado}&metodoPago=${metodoSeleccionado}&busqueda=${busquedaActual}&pagina=${i}"><c:out value="${i}"/></a>
                                                </c:when>
