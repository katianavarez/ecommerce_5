...
<html lang="es">
    <head>
        ...
        <title>Categorías — Velour Admin</title>
        ...
        <div class="admin-layout">
            <aside class="admin-sidebar">
                ...
                <nav class="admin-nav">
                    ...
                    <a href="${pageContext.request.contextPath}/admin/categorias" class="admin-nav__link admin-nav__link--active">
            ...
            <div class="admin-content">
                <div class="admin-topbar">
                    <div>
                        <h1 class="admin-topbar__title">Gestión de Categorías</h1>
                        <p class="admin-topbar__subtitle"><c:out value="${totalRegistros}"/> categorías registradas</p>
                    </div>
                    <div class="admin-topbar__actions">
                        <a href="${pageContext.request.contextPath}/admin/categorias?accion=nuevo" class="btn btn--gold btn--sm">+ Agregar categoría</a>
                    </div>
                </div>
                <div class="admin-body">
                    <c:if test="${not empty param.success}">
                        <div style="background:#d4edda;border:1px solid #c3e6cb;color:#155724;padding:var(--sp-3) var(--sp-4);border-radius:var(--r-md);margin-bottom:var(--sp-4);">
                            <c:choose>
                                <c:when test="${param.success == 'created'}">✓ Categoría creada.</c:when>
                                <c:when test="${param.success == 'updated'}">✓ Categoría actualizada.</c:when>
                                <c:when test="${param.success == 'deleted'}">✓ Categoría eliminada.</c:when>
                            </c:choose>
                        </div>
                    </c:if>
                            ...
                            <tbody>
                                <c:choose>
                                    <c:when test="${empty categorias}">
                                        <tr><td colspan="3" style="text-align:center;color:var(--text-muted);padding:var(--sp-8);">No hay categorías registradas.</td></tr>
                                    </c:when>
                                    <c:otherwise>
                                        <c:forEach var="cat" items="${categorias}">
                                            <c:set var="conteo" value="${conteoProductos[cat.id]}"/>
                                            <c:if test="${empty conteo}"><c:set var="conteo" value="0"/></c:if>
                                            <tr>
                                                <td>
                                                    <p class="admin-table__name"><c:out value="${cat.nombre}"/></p>
                                                    <p class="admin-table__sub">ID: ${cat.id}</p>
                                                </td>
                                                <td style="color:var(--text-muted);font-size:var(--fs-sm);">
                                                    <c:choose>
                                                        <c:when test="${conteo > 0}">
                                                            <a href="${pageContext.request.contextPath}/admin/productos?categoriaId=${cat.id}" style="color:var(--color-secondary);">
                                                                <c:out value="${conteo}"/> producto<c:if test="${conteo != 1}">s</c:if>
                                                            </a>
                                                        </c:when>
                                                        <c:otherwise>
                                                ...
                                                <td>
                                                    <div class="admin-table__actions" style="justify-content:flex-end;">
                                                        <a href="${pageContext.request.contextPath}/admin/categorias?accion=editar&id=${cat.id}" class="admin-action-btn" title="Editar">
                                ...
                                <c:if test="${totalPaginas > 1}">
                                    <nav class="pagination" style="margin-top:0;flex-shrink:0;">
                                        <c:if test="${paginaActual > 1}">
                                            <a class="pagination__btn" href="${pageContext.request.contextPath}/admin/categorias?pagina=${paginaActual - 1}">&#8249;</a>
                                        </c:if>
                                        <c:forEach var="i" begin="1" end="${totalPaginas}">
                                            <c:choose>
                                                ...
                                                <c:when test="${i == 1 || i == totalPaginas || (i >= paginaActual - 1 && i <= paginaActual + 1)}">
                                                    <a class="pagination__btn" href="${pageContext.request.contextPath}/admin/categorias?pagina=${i}"><c:out value="${i}"/></a>
                                                </c:when>
