...
<html lang="es">
    <head>
        ...
        <title><c:choose><c:when test="${not empty categoria.id and categoria.id > 0}">Editar</c:when><c:otherwise>Nueva</c:otherwise></c:choose> Categoría — Velour Admin</title>
            ...
            <div class="admin-content">
                <div class="admin-topbar">
                    <div>
                        <h1 class="admin-topbar__title">
                            <c:choose><c:when test="${not empty categoria.id and categoria.id > 0}">Editar Categoría</c:when><c:otherwise>Nueva Categoría</c:otherwise></c:choose>
                        </h1>
                        <p class="admin-topbar__subtitle"><a href="${pageContext.request.contextPath}/admin/categorias" style="color:var(--color-secondary);">← Volver a categorías</a></p>
                    </div>
                </div>
                <div class="admin-body">
                    ...
                    <form method="POST" action="${pageContext.request.contextPath}/admin/categorias">
                        <input type="hidden" name="accion" value="${not empty categoria.id and categoria.id > 0 ? 'actualizar' : 'crear'}">
                        <c:if test="${not empty categoria.id and categoria.id > 0}">
                            <input type="hidden" name="id" value="${categoria.id}">
                        </c:if>

                        <div class="admin-card">
                            <div class="admin-card__header"><h3 class="admin-card__title">Información de la categoría</h3></div>
                            <div class="admin-card__body">
                                <div class="product-form">
                                    <div class="form-group">
                                        <label class="form-label">Nombre *</label>
                                        <input class="form-control" type="text" name="nombre" value="<c:out value='${categoria.nombre}'/>" required maxlength="100" placeholder="Ej: Vestidos">
                                        <p style="font-size:var(--fs-xs);color:var(--text-muted);margin-top:4px;">Debe ser único. Máximo 100 caracteres.</p>
                                    </div>

                                    <div style="display:flex;gap:var(--sp-3);margin-top:var(--sp-5);">
                                        <button type="submit" class="btn btn--gold">
                                            <c:choose>
                                                <c:when test="${not empty categoria.id and categoria.id > 0}">Guardar cambios</c:when>
                                                <c:otherwise>Crear categoría</c:otherwise>
                                            </c:choose>
                                        </button>
                                        <a href="${pageContext.request.contextPath}/admin/categorias" class="btn btn--ghost">Cancelar</a>
                                    </div>
                                </div>
                            </div>
                        </div>
